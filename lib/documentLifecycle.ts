...
export async function reverseSplit(input: ReverseSplitInput): Promise<ReverseResult> {
  ...
  // Mark the source's document_supersessions rows as historical by
  // deleting them — the audit log retains the relationship, so
  // history is reconstructable. (Hard-delete the join row, not the docs.)
  await supabase
...
export async function reverseRenumber(input: ReverseRenumberInput): Promise<ReverseResult> {
  ...
  // Make sure the doc still has the renumbered value before we
  // swap it back, otherwise something else changed it in between
  // and we shouldn't blindly overwrite.
