...
// ─── AI assist strip (separate file, ships in commit B) ────────
//
// Stub here so this commit compiles without it. The real
// implementation reads notes' bodies and calls into lib/ai when a
// provider is configured. With no provider, the strip renders
// nothing (per the directive: "degrade gracefully if keys missing").

function AiAssistStrip({ notes }: { notes: Note[] }) {
  // Will be replaced in commit B with the real AI seam. For this
  // commit the strip is intentionally absent so the scratchpad
  // works fully without any AI plumbing.
  void notes;
  return null;
}
