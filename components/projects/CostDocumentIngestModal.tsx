...
const KINDS: CostDocumentKind[] = ["quote", "estimate", "po", "subcontract", "invoice", "change_order", "other"];
...
interface Row {
  ...
  amount: number;
  costType: CostType;
  /** Existing account id, or "new" to create one of this cost type. */
  accountId: string;
}
...
export default function CostDocumentIngestModal({
  ...
  const [result, setResult] = useState<{ createdAccounts: number; postedEntries: number; postedTotal: number } | null>(null);
  ...
  const onExtract = async () => {
    ...
    try {
      ...
      const plan = planFromExtraction({ ...ext, kind: effectiveKind }, accounts, { partyId: partyId || null });
  ...
  const onPost = async () => {
    ...
    try {
      ...
      const newAccounts = dedupeNewAccounts(rows);
      const res = await postIngestPlan({
        ...
        plan: {
          ...
          newAccounts,
          lines: rows.map((r) => ({
            line: { description: r.description, amount: r.amount, costType: r.costType },
            accountId: r.accountId === "new" ? null : r.accountId,
            newAccount: r.accountId === "new" ? { name: COST_TYPE_LABEL[r.costType], costType: r.costType } : undefined,
          })),
        },
        ...
        {step === "review" && (
          <div className="space-y-3">
            ...
            <div className="rounded-xl border border-[var(--color-border)] overflow-hidden">
              <div className="grid grid-cols-[1fr_90px_110px_1fr_28px] gap-2 px-3 py-2 bg-[var(--color-surface-2)] text-[9px] font-black uppercase tracking-widest text-[var(--color-text-muted)]">
                <span>Description</span><span className="text-right">Amount</span><span>Cost type</span><span>Maps to account</span><span />
              </div>
              <div className="divide-y divide-[var(--color-border)] max-h-72 overflow-auto">
                {rows.map((r, i) => {
                  ...
                  return (
                    <div key={i} className="grid grid-cols-[1fr_90px_110px_1fr_28px] gap-2 px-3 py-2 items-center">
                      ...
                      <select value={r.costType} onChange={(e) => setRow(i, { costType: e.target.value as CostType, accountId: "new" })}
                        className="text-xs px-1.5 py-1 rounded border border-[var(--color-border)] bg-[var(--color-surface)]">
                      ...
                      <select value={r.accountId} onChange={(e) => setRow(i, { accountId: e.target.value })}
                        ...
                        <option value="new">➕ New {COST_TYPE_LABEL[r.costType]} account</option>
                      </select>
              ...
              <button onClick={() => setRows((rs) => [...rs, { description: "", amount: 0, costType: "odc", accountId: "new" }])}
...
const KIND_LABEL: Record<CostDocumentKind, string> = {
  quote: "Quote", estimate: "Estimate", po: "Purchase order", subcontract: "Subcontract",
...
function dedupeNewAccounts(rows: Row[]): Array<{ name: string; costType: CostType }> {
  const seen = new Set<CostType>();
  const out: Array<{ name: string; costType: CostType }> = [];
  for (const r of rows) {
    if (r.accountId === "new" && !seen.has(r.costType)) {
      seen.add(r.costType);
      out.push({ name: COST_TYPE_LABEL[r.costType], costType: r.costType });
    }
  }
  return out;
}
