...
export default function ExecutionGuide() {
  ...
  return (
    <>
      ...
      {/* First-run banner. */}
      {!bannerDismissed && (
        <div className="w-full flex items-center gap-2 bg-gradient-to-r from-[var(--color-accent-soft)] to-white border border-[var(--color-accent-ring)]/40 rounded-lg px-3 py-2 text-xs text-slate-700">
          ...
          <button onClick={markSeen} className="p-0.5 rounded hover:bg-black/5 text-slate-400 transition-colors" title="Dismiss"><XIcon className="w-3.5 h-3.5" /></button>
        </div>
      )}
      ...
      {open && (
        <div className="fixed inset-0 z-[300] flex items-start sm:items-center justify-center overflow-y-auto p-4" onClick={() => setOpen(false)}>
          ...
          <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl ring-1 ring-slate-900/10 overflow-hidden max-h-[85vh] flex flex-col animate-in fade-in zoom-in-95" onClick={(e) => e.stopPropagation()}>
            <div className="px-5 py-4 border-b border-slate-200 flex items-center gap-2 bg-gradient-to-b from-white to-slate-50/50">
              ...
              <button onClick={() => setOpen(false)} className="ml-auto p-1.5 rounded-md text-slate-500 hover:bg-slate-100 transition-colors"><XIcon className="w-4 h-4" /></button>
            </div>
