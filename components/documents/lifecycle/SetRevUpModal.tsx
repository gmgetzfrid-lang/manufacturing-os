      ...
      <div className="w-full max-w-3xl bg-white rounded-2xl shadow-2xl overflow-hidden my-8 animate-in fade-in zoom-in-95">
        <div className="px-5 py-4 border-b border-slate-200 flex items-center justify-between bg-slate-50">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="font-black text-slate-900">Bump Set Revision</h2>
              <div className="text-[11px] text-slate-500 mt-0.5">Every active sheet in this set, in one operation.</div>
            </div>
          </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 space-y-2">
            <div className="text-[10px] font-black text-slate-700 uppercase tracking-widest">Shared across all sheets</div>
            <div className="grid grid-cols-2 gap-2">
              <select value={issueType ?? ""} onChange={(e) => setIssueType(e.target.value as DocumentVersion["issueType"])} className="text-xs border border-slate-300 rounded px-2 py-1.5 bg-white">
                {ISSUE_TYPES.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
              </select>
              <select value={changeType ?? ""} onChange={(e) => setChangeType(e.target.value as DocumentVersion["changeType"])} className="text-xs border border-slate-300 rounded px-2 py-1.5 bg-white">
                {CHANGE_TYPES.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
              </select>
            </div>
            <input value={sharedMoc} onChange={(e) => setSharedMoc(e.target.value)} placeholder="MOC reference" className="w-full text-xs border border-slate-300 rounded px-2 py-1.5 font-mono" />
            <textarea
          ...
          {/* Sheets */}
          <div>
            <div className="text-[10px] font-black text-slate-700 uppercase tracking-widest mb-1">Sheets ({sheets.length})</div>
            {loading ? (
              ...
              <div className="text-xs text-slate-500 italic py-6 text-center border border-dashed border-slate-200 rounded">
                ...
                {sheets.map((s, i) => (
                  <div key={s.doc.id} className="px-3 py-2 grid grid-cols-12 gap-2 items-center text-xs">
                    <div className="col-span-3 min-w-0">
                      <div className="font-mono font-bold text-slate-800 truncate">{s.doc.documentNumber || "—"}</div>
                      <div className="text-[10px] text-slate-500 truncate">{s.doc.title || s.doc.name}</div>
                    </div>
                    <div className="col-span-2 font-mono text-slate-500">Rev {s.doc.rev || "—"}</div>
                    ...
                    <div className="col-span-5">
                      <label className="block border border-dashed border-slate-300 rounded p-1 cursor-pointer hover:border-emerald-400 hover:bg-emerald-50/30 text-center text-[11px]">
