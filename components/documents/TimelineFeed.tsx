...
export default function TimelineFeed({ events, showScope = true, emptyMessage = "No history yet." }: TimelineFeedProps) {
  if (events.length === 0) {
    return (
  ...
  return (
    <div className="relative">
      ...
      <div className="space-y-3">
        {events.map((e) => (
          <TimelineRow key={e.id} event={e} showScope={showScope} />
        ))}
      </div>
    </div>
  );
...
function TimelineRow({ event, showScope }: { event: TimelineEvent; showScope: boolean }) {
