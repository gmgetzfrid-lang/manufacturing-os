...
export default function DocThumb({
  ...
  React.useEffect(() => {
    if (!visible || !filePath || !isPdf) return;
    let alive = true;
    void resolveUrl(filePath).then((u) => { if (alive) { if (u) setUrl(u); else setFailed(true); } });
    return () => { alive = false; };
  }, [visible, filePath, isPdf]);
  ...
  // Non-PDF, no file, or render failure → typed placeholder.
  if (!filePath || !isPdf || failed) {
