...
// LifecycleBoard — the document-flow board, hosted on /coordination as its
// "Document flow" view. Every controlled document renders as a card in its
...
// Includes the protection-record strip and (for controllers) the Doc Control
// review queue, which ride with the board wherever it lives.

import React, { useCallback, useEffect, useMemo, useState } from "react";
import Link from "next/link";
import { RefreshCw, Lock, AlertTriangle, Search } from "lucide-react";
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-3 flex-wrap">
        <p className="text-xs text-[var(--color-text-muted)] flex-1 min-w-[200px]">
          Every controlled document by lifecycle state. Hotter age chips = sitting longer — that&apos;s your bottleneck.
        </p>
      ...
      <div className="overflow-x-auto">
        <div className="flex gap-3 min-w-max">
          {COLUMNS.map((col) => {
            ...
            return (
              <div key={col} className={`w-72 shrink-0 rounded-2xl border ${COLUMN_TONE[col]} flex flex-col h-[calc(100dvh-260px)]`}>
