...
--
--   1. ai_key_agreements — the signed record behind every saved API key:
--      who accepted which version of the data-handling agreement, for which
--      key (last 4), when, from where. Service-role only (written by the
--      connection route, read back through the API) — airtight, not
--      client-editable.
...
--
-- Personal-key provider restriction (Claude/OpenAI only) is code-enforced
-- in the connection + ask routes; no schema needed.
--
