...
| **Title-block ingest** | `lib/titleBlock.ts` / `titleBlockHeuristics.ts`: the upload staging modal now reads page-1 PDF text and fills drawing numbers + revisions the filename didn't carry (confidence-gated; user edits always win). Strictly subordinate to the upload it precedes — each read is capped at 10s, the whole pass aborts when the modal closes or the batch changes, and it stands down entirely while `lib/uploadActivity` reports a transfer in flight. |
| **Bulk upload control** | Uploads are cancellable end to end: `uploadToPath({ signal })` → `putWithXhr` closes the socket and rejects `UploadCancelledError`; multipart stops rather than burning its retries. The staging modal's X is never disabled and always aborts first, `submitting` is cleared in a `finally` (a stuck flag survived between batches and made the *next* batch open pre-wedged), and a stopped batch reports landed / failed / never-started separately. |
| **pdf.js verbosity** | `lib/pdfjsConfig.ts`: every `getDocument` and every react-pdf `<Document options>` runs at `VerbosityLevel.ERRORS`. AutoCAD-exported drawings emit `TT: undefined function: N` once per unmappable glyph — harmless, and enough of them to bury real errors during a bulk stage. |
