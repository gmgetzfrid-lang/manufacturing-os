...
- **Independently verified:** ✓ **SURVIVES** — independent adversarial pass. Fully confirmed; I could find no compensating guard. `grep -n "loadPrincipal|readableControlledDocIds|ai_excluded" app/api/graph/ask/route.ts` returns nothing, and the entity_mentions read at :138-139 selects `context_snippet` with no readable-set filter either. The /graph page has no role gate at all (no role/Viewer/redirect references in app/(protected)/graph/page.tsx), so any active member reaches the ask box at :278. SECURITY INVOKER is genuinely inverted here: it inherits the service role's RLS-bypass rather than constraining anything.
...
- **Re-verified:** hardening pass — **SURVIVES**, and the file documents its own gap. The only content boundary applied is `.eq("ai_excluded", false)` (`:101`), and the comment at `:137-138` states plainly that *"graph_ask runs on the service role here."* No document ACL, visibility or discoverability test is applied on any read tool.
- **Independently verified:** ✓ **SURVIVES** — independent adversarial pass. The ToolContext (tools.ts:26-32) carries `role` but no team/ACL data and nothing in the file ever calls loadPrincipal/readableControlledDocIds. The route gate (app/api/orchestrator/route.ts:61-66) requires only `status = 'active'` and defaults `role` to Viewer — no controller restriction — so any active member with their own API key gets service-role-wide retrieval. The file's own stated rule 1 ("NOTHING WIDENS ACCESS", :10-12) is violated.
...
- **Re-verified:** hardening pass — **SURVIVES**. `answer_skills_insert` is `WITH CHECK (active member AND created_by = auth.uid())` — no role term — and the SELECT admits `visibility = 'org'`. A Viewer can publish an org-wide skill.
- **Independently verified:** ✓ **SURVIVES** — independent adversarial pass. Verified there is no server-side authoring route and no role gate: /intelligence/skills computes `canManageOrg` (page.tsx:52) only for toggle/delete affordances, while the 'Build a skill' card renders on `activeOrgId && uid` alone. Placement is `baseAnswerSystem + orgInstructions + buildPagesNote(...)` (route.ts:1536), i.e. after the citation/safety rules — the claim's 'appended to the END' is right modulo the trailing pages note; the only defence is the prompt sentence 'they never override the citation and safety rules above'.
...
- **Independently verified:** ✓ **SURVIVES** — independent adversarial pass. Repo-wide grep over knowledge_questions shows only inserts (ask route :304,:310), a rating update (feedback route) and reads — no revision column, no invalidation on rev-up, no filter on source_rev anywhere. The proven-ground block filters excludedDocIds (ACL) but nothing about revision, so a Rev-A citation to page 12 silently pulls whatever Rev B now has on page 12 and seats it as 'governing'; the memory card at app/(protected)/knowledge/[id]/page.tsx:1703-1719 replays the Rev-A prose verbatim with its stale citations.
...
- **Independently verified:** ✓ **SURVIVES** — independent adversarial pass. Confirmed end to end: the card at page.tsx:1697-1721 renders `pa.answer` with its citations and `setThread([{ question: pa.question, answer: past }])`, and the next ask ships `history: thread.slice(-4)` back into the model (page.tsx:1424-1427); openConversation (:1497-1514) reopens whole threads the same way. Answers computed under the asker-specific excludedDocIds filter are therefore replayable by members who fail that filter.
...
- **Independently verified:** ✓ **SURVIVES** — independent adversarial pass. Read policy is membership-only with no ACL restrictive policy on entity_mentions (the 20260708 restrictive policies cover documents/collections only), and the two consumers — components/assets/MentionsPanel.tsx and components/graph/NodePeek.tsx — call mentionsForAsset/mentionsForDocument directly with no readability filter of any kind. Document name plus proving sentence reach any member.
...
- **Re-verified:** hardening pass — **SURVIVES**. `process_flows_insert` is `WITH CHECK (active member AND created_by = auth.uid())`. Nothing constrains `from_ref`/`to_ref`, so any member can assert an arbitrary edge into the plant topology.
- **Independently verified:** ✓ **SURVIVES, corrected** — independent adversarial pass. Severity **HIGH → MEDIUM** by this pass. The RLS gap is real and the API-vs-DB asymmetry is exactly as described. Severity lowered because the 'plant topology is forgeable' half is the product's intended design, not a hole: lib/processFlows.ts:52-69 createManualFlow lets ANY member insert `status: "confirmed"` from the graph UI already (app/(protected)/graph/page.tsx:335-336, ungated). The genuine delta over the sanctioned path is forged PROVENANCE — `origin:'ai'`, a real `source_document_id`/`source_page`, and a fabricated `decided_by_name` on a row no controller ever saw (the accept/dismiss UI is isAdmin-gated, components/assets/UnitOpsPanels.tsx:209).
...
- **Independently verified:** ✓ **SURVIVES** — independent adversarial pass. Both halves confirmed. Nothing in the repo ever deletes or ages out dismissed rows (listProcessFlows merely hides them with `.neq("status", "dismissed")`), so a Rev-0 dismissal permanently suppresses that pair on every later revision, reported to the operator only as the aggregate `skippedSettled` and the misleading note 'No new flows found' (:179). The all-or-nothing insert against the UNIQUE (org_id, from_kind, from_ref, to_kind, to_ref) index (migration :36) loses the entire batch on any collision — reachable via a concurrent run or a manual draw landing between the :82 read and the :170 write — after the vision call has already been metered.
...
- **Re-verified:** hardening pass — **SURVIVES**. `source_document_id UUID REFERENCES knowledge_documents(id) ON DELETE SET NULL` (`20261017_process_flows.sql:27`). The flow row survives its own provenance; an AI-asserted edge becomes indistinguishable from a hand-drawn one.
- **Independently verified:** ⛔ **REFUTED** by an independent adversarial pass — do not work this finding. Kept in place with the reason rather than deleted (`DEC-41`). The FK nulling is real, but the stated harm ('where did this come from?' → nothing) is false: the document NAME and PAGE persist in evidence/source_page and are what the UI actually displays. Moreover a repo-wide grep shows NO reader of `process_flows.source_document_id` anywhere in app/, components/ or lib/ (the only `source_document_id` reads are on knowledge_documents, in flows/browse), so nulling it changes nothing a user or any code path can observe — and confirmed flows never showed provenance at all, deletion or not.
...
- **Re-verified:** hardening pass — **SURVIVES**. `knowledge_documents` and `knowledge_chunks` are both in `ORG_SCOPED_TABLES` (`exportTables.ts:138-139`), and the export gate admits `["Admin", "Manager", "DocCtrl"]` (`structured/route.ts:55`). Chunk text is document prose, so the whole corpus leaves through a role that never had document-level read.
- **Independently verified:** ✓ **SURVIVES** — independent adversarial pass. Confirmed. A grep for acl|visibility|is_private|redact across all 363 lines of lib/dataExport.ts matches nothing (the only hits are the word 'audit'), so the dump is by org_id with no filter. The audit row that IS written (dataExport.ts:181-196, action DATA_EXPORT) records only tableCount/totalRows/fileCount/totalBytes/startedAt — no role and no ACL-filtered flag — which is exactly the gap the finding's third 'Done when' names.
...
- **Independently verified:** ✓ **SURVIVES** — independent adversarial pass. Repo-wide grep for the literal `answered` finds no other producer — no AI call, no key lookup, no branch anywhere in the route; the string only appears in the type union on line 50. The claim of absence is confirmed. Only mitigation worth noting: the sole consumer (app/(protected)/graph/page.tsx:51-57) declares its own GraphAsk interface without `mode`/`answer`, so no client branch is dead TODAY and no UI copy promises a written answer — the defect is a false exported contract, not a broken screen.
...
- **Re-verified:** hardening pass — **SURVIVES**. `chainReadable` opens `if (principal.isController) return true` then walks the ACL chain (`knowledgeAccess.ts:62-64`); the SQL evaluator opens `IF p_visibility IS NULL OR p_visibility = 'normal' THEN RETURN true` (`20260708_acl_rls_enforcement.sql:53-55`). Different first questions, so different answers for the same document.
- **Independently verified:** ✓ **SURVIVES** — independent adversarial pass. Confirmed on every leg: the key is obtainable because lib/knowledgeSourceSync.ts:231 mirrors `file_key: version.file_url` into knowledge_documents, whose SELECT policy (20260911_knowledge_ai.sql:125-128) is plain org-membership with no ACL restrictive policy attached. The only other gate at :91-111 fires solely on explicit `deny.*.download` entries, so a normal-visibility, allow-listed folder yields signed bytes to any active member.
