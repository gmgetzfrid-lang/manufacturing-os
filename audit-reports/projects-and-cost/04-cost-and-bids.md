...
- **Independently verified:** ✓ **SURVIVES** — second independent adversarial pass. Confirmed on every cited line. The one place currency survives is the invoice row's own display (QuotesPanel.tsx:154 `fmtMoney(doc.totalAmount, doc.currency ?? "USD")`) — the bid tab, the award confirm dialog, the commitment entry and the rollup all lose it.
...
- **Independently verified:** ✓ **SURVIVES** — second independent adversarial pass. Confirmed. The only surviving trace of how much was read is the audit row's `pagesRead` (:153); no UI surface anywhere warns that a 14-page quote was read to page 8, and nothing compares pages rendered against the PDF's true page count.
