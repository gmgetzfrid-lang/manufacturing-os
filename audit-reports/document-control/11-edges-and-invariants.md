...
- **Independently verified:** ✓ **SURVIVES** — independent adversarial pass. Confirmed and exploitable end to end: the needsMapping branch (:145-156) returns `headers`, `sheetNames` and `rowCount` of the foreign object BEFORE any AI-key gate, and because `body.columnMap` is merged last at :143 the caller can map any header of the victim workbook onto their own template's data tags, so :169 `v[p.tag] = row[columnMap[p.tag] ?? ""]` returns the actual cell contents in `documents[].values`. The same feature's analyze action enforces the prefix check with a comment naming this exact risk, which makes the omission a genuine gap rather than a design choice.
...
- **Independently verified:** ✓ **SURVIVES** — independent adversarial pass. I tried to refute this by checking whether the branch is reachable and whether the header layer tolerates U+2014; both went against the claim. lib/outputTemplates.ts:261-270 renderDocuments() posts action "render" WITHOUT returnJson, so the early JSON return at :360 is skipped and any batch of 2+ documents reaches :384. The throw is unconditional — every multi-document download 500s after output_generations and OUTPUT_DOCS_GENERATED already claim success. renderFilename (lib/outputTemplateText.ts:120-138) also strips only [\\/:*?"<>|], so the single-file path at :375 throws the same way on any non-Latin-1 character in a filename value.
...
- **Independently verified:** ✓ **SURVIVES** — independent adversarial pass. Confirmed, and I could find no compensating filter: there is no bucket-ownership check, no key-shape test, and no dry-run. Retention is applied unconditionally after every successful s3/r2 push, so the first scheduled run against a shared corporate bucket with a blank Prefix permanently deletes every object in it older than retention_days, in 1000-key batches (:398-404).
...
- **Independently verified:** ✓ **SURVIVES, corrected** — independent adversarial pass. Severity **HIGH → MEDIUM** by this pass. The factual claim is correct: transmittals bypass publicOrigin() entirely, so NEXT_PUBLIC_SITE_URL is ignored even when set, and the QR/email link inherits whatever host the controller happens to be browsing. I'd lower it to MEDIUM because the failure is conditional on deployment topology — on the ordinary single-canonical-domain production deploy window.location.origin and publicOrigin() are identical and nothing breaks; the damage requires the sheet to be printed from a preview/branch/non-canonical host. That is narrower than the app-wide, always-on exposure the other HIGHs describe.
...
- **Independently verified:** ✓ **SURVIVES** — independent adversarial pass. Confirmed by reading the handler end to end — the only gate on the route is the CRON_SECRET bearer check at :52-55, which authenticates the caller, not the workspace's entitlement. lib/serverAuth's authorizeOrgRole is not imported here at all, so the SubscriptionGate/authorize checks that guard the interactive export routes have no equivalent on the scheduled path.
...
- **Independently verified:** ✓ **SURVIVES** — independent adversarial pass. Verified by reading the whole [id]/route.ts: the only authorization is authorizeOrgRole(req, orgId, ADMIN_ROLES) at :45, identical to create, and the orgs table is never queried. PATCH also recomputes next_run_at at :73-79, so the same call that adds the bucket arms the schedule the free-tier org was not supposed to have.
...
- **Independently verified:** ✓ **SURVIVES** — independent adversarial pass. I looked specifically for a compensating control and found none: grep across lib/, app/ and next.config for setGlobalDispatcher, undici, or redirect:"manual" returns zero hits, so nothing constrains redirects globally. The guard is also applied only to endpoint/webhook_url — the S3 path at :247 validates dest.endpoint but the AWS SDK then resolves and connects independently, giving the same TOCTOU window.
...
- **Independently verified:** ✓ **SURVIVES** — independent adversarial pass. Confirmed, and the contrast inside the contract itself sharpens it: EXPORT_EXCLUDED_TABLES (exportTables.ts:173-174) excludes ai_connections with the reason "holds live AI provider API keys — secrets never leave the database", while export_destinations' credentials do leave, in the same ZIP. MEDIUM is fair — the restored org never sees plaintext, but buildS3ClientFromDestination (exportRunner.ts:333-342) decrypts and uses them, so org B gets a working confused-deputy handle on org A's bucket.
...
- **Independently verified:** ✓ **SURVIVES** — independent adversarial pass. Both halves check out, and the reachability claim does too — the Generate button at output-templates/page.tsx renders for every member (`<Button size="sm" onClick={() => setGenerating(t)} disabled={!t.templateFileKey}>`), while the Edit/Delete controls sit behind `{isController && ...}`. MEDIUM is the right level: the arbitrary tag→value map is largely inert (nullGetter at docxRender.ts:79 makes unknown keys render empty and extra keys are ignored), so the real exposure is the unbounded render loop and in-RAM zip against the 300s maxDuration.
...
- **Independently verified:** ✓ **SURVIVES** — independent adversarial pass. Survives. The reachability half is if anything understated: because XEDGE-1 leaves sourceFileKey unvalidated, the crafted workbook need not even be the caller's own upload, and XLSX.read runs in the nodejs runtime (route.ts:42) alongside supabaseAdmin's service-role client and the R2 credentials. MEDIUM is right — it is a known-vulnerable dependency with a real path to it, not a demonstrated exploit.
...
- **Independently verified:** ✓ **SURVIVES** — independent adversarial pass. Confirmed. I checked the file's own stated safety layers and none of them close this: the fail-closed guard at :95-97 only catches query ERRORS, not silently-skipped rows; MIN_AGE_DAYS=7 (:25) does not protect a current published PDF whose object is older than a week; PROTECTED_PREFIXES (:26) covers only data/ and exports/; and the 're-scan server-side' mitigation at :157 re-runs the same unordered scan, so it reproduces the flaw rather than correcting it.
...
- **Independently verified:** ✓ **SURVIVES** — independent adversarial pass. Survives, and the absence claim holds under a repo-wide grep: the only writer of subscribed_plan anywhere in app/, lib/ or components/ is webhook/route.ts:63 — every other hit is a read (destinations/route.ts:85-86, SubscriptionProvider.tsx:41, admin/settings/page.tsx:154). So a portal-side plan change never reaches the app, and any subscription.updated event whose metadata lacks `plan` nulls the column and immediately trips the Growth gate at destinations/route.ts:88-93.
