...
- **Independently verified:** ✓ **SURVIVES** — second independent adversarial pass. Right as written. The only distinguishing artifact is the history label 'Approved with minor correction — issued Rev N'; the status, the issued rev, and the notification classification (ticketTransitions.ts:344-347 groups it with the approvals) are identical, so the engineer gate is one click wide for exactly the users it targets.
...
- **Independently verified:** ✓ **SURVIVES** — second independent adversarial pass. Confirmed. The server-side enforcement in /api/tickets/workflow-action is real but entirely optional — the anon-key REST endpoint accepts a direct PATCH of status/deliverable_rev/requester_role/attachments from any active member of the org.
...
- **Independently verified:** ✓ **SURVIVES** — second independent adversarial pass. Confirmed — grep shows every writer of requester_role is client-side (requests/new/page.tsx:309, CheckInPanel.tsx:250, transitionIn.ts:315); no API route or DB constraint ever validates it against org_members.role. Note this is partly subsumed by AUTHZ-2 (with a blanket write policy the field can also just be UPDATEd after the fact), but it stands independently as an unvalidated authority input.
...
- **Independently verified:** ✓ **SURVIVES** — second independent adversarial pass. Confirmed by direct comparison — resolve has no document lookup for authorization at all (its document_versions read at 32-38 only fetches archived_at/archive_id), so a hidden/private document or one with an explicit download deny is signable through it by any active member.
...
- **Independently verified:** ✓ **SURVIVES** — second independent adversarial pass. Confirmed, and the attachments array is member-writable by the blanket tickets policy plus the client write at requests/[id]/page.tsx:1010-1014. The deletes are irreversible (DeleteObjects) and the admin sees only a `keysDeleted` count in the audit row (line 202), so a planted key destroying another tenant's — or the org's own controlled document's — binary is invisible.
...
- **Independently verified:** ✓ **SURVIVES** — second independent adversarial pass. Correct as stated — the identity check `isRequesterIdentity` is ORed with, not ANDed to, the role capability, so the role grants org-wide requester authority over every ticket. Worth noting the workflow-action route re-derives the same getActions server-side (route.ts:96), so this is enforced authority, not just UI.
...
- **Independently verified:** ✓ **SURVIVES** — second independent adversarial pass. Confirmed in both directions: DocCtrl reaches the page but is denied by the RLS restrictive policy on the upsert (page.tsx:97-100 upsert, caught by the generic `catch` that only shows "Failed to save configuration."), while Manager/Supervisor/DraftingSupervisor satisfy the DB policy but are redirected out of the page. No other writer of key='drafting' exists (only lib/ticketRouting.ts:49 reads it), so there is no alternate path that rescues DocCtrl.
...
- **Independently verified:** ✓ **SURVIVES, corrected** — second independent adversarial pass. Severity **MEDIUM → LOW** by this pass. Every literal sub-claim checks out: internal workflow status, unit and last-activity timestamp are returned forever to any holder of the ticket UUID, unrated and with no archived/retired check (unlike app/api/verify/route.ts:88 which computes `docRetired` from status). Severity lowered to LOW: exposure is a handful of low-sensitivity fields for one ticket, gated behind an unguessable UUID that the org itself printed on a deliverable it handed to that party — no files, URLs, or people are returned.
