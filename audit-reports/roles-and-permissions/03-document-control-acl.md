...
**Resolution (2026-09-17, Round E) — accepted by design, `DEC-43`.** Controllers stay unscoped. Reasoning, per the WONTFIX bar (DEC-28):
- **Cost of fixing:** the record's remediation is a per-controller library scope — a new column on `org_members`, a scope check inside `node_visible` before the bypass (the function that gates ALL document read visibility, which DEC-2 says to land last and alone), the same in the `20260901` write guards' `is_org_controller` exemptions, a member-editor surface and a simulator branch. Five files at minimum, a change to the one function every read goes through, and a new way for an org to lock its own controllers out of the documents they must be able to recover — for a MEDIUM whose exposure is that a document controller sees the documents.
- **Rejected alternative:** the split (Admin unconditional, DocCtrl scoped). A scoped DocCtrl is not a controller: `DEC-2` routes the publish path, the review guard, ack rows and `node_visible` itself through `is_org_controller` precisely so the collection has a recovery rail that cannot be configured away. Scoping DocCtrl re-opens the class of "the person who must fix this cannot see it".
- **What would change the answer:** a stated requirement for per-area document controllers (none stated), together with DEC-5's stable role ids so a scope could be stored against something other than a bare string.
- **Mitigation landed (the one cheap thing):** a controller read of a RESTRICTED node that only the bypass allowed is now recorded. `lib/permissions.ts` `controllerBypassDecided(...)` is pure: true when the principal is a controller by collection AND the same person stripped of the controller roles would not be served (the ACL itself does not admit them). `app/api/storage/download-url/route.ts` — the bytes egress point — evaluates it beside the existing `canServeContent` decision on private/hidden documents, with the document's explicit owner in the evaluation (`effectiveOwnerUserId`, GAP-15/DEC-7) and, when the bypass decided the read, asks `user_is_effective_owner` (the folder / library / team cascade) before writing — an owner-controller's download of their own document leaves no row, per DEC-43's acceptance — and writes an `audit_logs` row `CONTROLLER_RESTRICTED_READ` (`resource_type: "document"`, details: path, visibility, roles), best-effort so a failed insert never blocks the rail (a cascade lookup error records the read rather than skipping it). Reads through PostgREST (`node_visible`) cannot be audited from inside an RLS function without a per-row side effect on every SELECT — deliberately not attempted.
- Tests: `lib/__tests__/roundE_D_rolesAdmin.test.ts` — "DOCACL-3 / DEC-43" describe: `controllerBypassDecided` true only when the controller tier is the sole reason (not when another held role is granted, not for a non-controller, not on a normal-visibility node, not for the effective owner); the served decision itself unchanged; the route's audit row and best-effort shape pinned, plus `effectiveOwnerUserId` inside `contentCheck` and the cascade RPC consulted before the insert. `lib/__tests__/sweepRoundA3.test.ts` pin updated to the shared `contentCheck`.

**Done-when.**
1. ✗ A `DocCtrl` can be scoped to one or more libraries — not done, by decision (DEC-43).
2. ✓ An unscoped controller behaves exactly as today (nothing changed for them; the audit row is additional).
3. ✗ The scope is visible in the member editor and the simulator — no scope exists to show.

**Scope / residual.** No migration (`audit_logs` inserts are service-role from the route). The audit page shows the new action under its document resource type.
