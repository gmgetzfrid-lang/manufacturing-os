...
**Done-when.**
1. ✓ Narrowing the capability actually restricts the data — on the page's path (the service-role route refuses before any read). Stated residual: the `tickets` SELECT policy is still org-wide, so a member can read tickets through PostgREST regardless of this capability — that is `WF-9`'s move-behind-routes half (recorded as WF-2's residual), not this finding's.
...
**Scope / residual.** Files: `lib/adminSurfaces.ts`, `lib/adminGate.ts`, `lib/capabilityPolicy.ts` (strict loader), `app/api/admin/gate/route.ts`, `app/api/admin/analytics/route.ts`, `app/(protected)/admin/layout.tsx`, the two pages. No migration for this finding.
