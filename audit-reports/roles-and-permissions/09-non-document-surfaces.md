...
**Resolution (2026-09-02, Phase 6 severity sweep, Round C3).** The signing ceremony is minted by the server. `/api/signatures/sign` (new) is the only writer of `e_signatures`: it takes the bearer's user, **verifies the re-authentication itself** — a password account's password is probed against Supabase Auth with a throwaway client (the session it issues is discarded, and it must resolve to the same user as the bearer), an SSO account's provider sign-in must fall inside the 15-minute freshness window read from the token's own user record, never from a client claim — derives `signer_name` / `signer_role` / `signer_email` from the membership row (client-supplied identity is ignored), takes `content_hash` from the version row the publish path hashed (a disagreeing client hash is refused, 409), gates `Approved` on approval capability by the role collection, writes the row on the service-role key with `reauth_method` / `reauth_at`, and mirrors `ESIGNATURE_CAPTURED` as a *checked* audit write. The browser no longer verifies anything: `SignatureCeremony` sends the credential with the signature; `recordSignature` calls the route; `verifySigningCredential` (the mid-ceremony `signInWithPassword` that rotated the session) is gone. `20261050` drops `e_signatures_self_insert`, adds no INSERT policy, and installs `enforce_signature_ceremony` (BEFORE INSERT: any user-JWT insert is refused; service pass) so the invariant survives a future policy. Reading is unchanged; signatures stay immutable. The roster guards (`SURF-12` / `SURF-13`) keep binding `signature_id` to `auth.uid()` on the roster UPDATE the browser still performs — the route stamps `signer_user_id` from the bearer, so their predicates hold as before. The panel renders how each signature was re-authenticated; the typed-name confirmation compares against the membership display name.
- Done-when: ✓ signature creation verifies a fresh re-authentication assertion server-side before inserting; ✓ the direct client INSERT path is closed (policy dropped + trigger).
- Cross-area: the same code closes document-control [`RG-9`](../document-control/03-review-gate.md) and drafting-flow [`EVID-3`](../drafting-flow/10-audit-evidence.md); their records point here.
- Migration: `20261050` — **applied & verified live 2026-09-02** (5/5 probes true; inventory recorded BEFORE apply: 1 e-signature row in total, 0 in the last 30 days, 2 policies on the table, 1 INSERT policy — the client path closed with nothing in flight). Files: `app/api/signatures/sign/route.ts` (new), `lib/eSignatures.ts`, `components/signatures/SignatureCeremony.tsx`, `components/signatures/SignatureCaptureHost.tsx`, `components/signatures/SignaturePanel.tsx`, `components/documents/ReviewGateSection.tsx`, `components/documents/AckSection.tsx`, `lib/reviewControl.ts`, `lib/acknowledgments.ts`, `components/providers/RoleContext.tsx` (member display name), `lib/dataRestore.ts` (reason text). Tests: `lib/__tests__/sweepRoundC3.test.ts` (the route driven with a mocked auth probe: wrong password, stale SSO, forged identity, hash disagreement, intent authority; source pins; `20261050` shape).
