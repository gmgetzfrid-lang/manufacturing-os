...
**Done-when.** ✓ 1. the four writers confirm the row was written before returning. ✓ 2. audit row and notification only after that confirmation. ✓ 3. a refused write is a visible error at every UI caller (nine call sites across five files — eight pinned individually, and the ninth, `/admin/libraries` via the `assignLibraryOwner` alias, surfaces the writer's own refusal text in the page's save error).
