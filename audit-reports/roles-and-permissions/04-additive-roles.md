...
**Amendment (2026-09-02, Phase 6 severity sweep, Round C1b).** The Phase 5.5 closure converted the pages it named and left the rest to a later census. That census (an exhaustive recon over `app/(protected)/**`, `components/**`, `app/api/**` and `lib/**`) found **42 further headline-keyed client affordance gates across 27 files** — fifteen admin pages (assets, ai-instructions, libraries, branding, restore, proposed-links, storage, permissions, audit, codebook, holds, scope, data-export, billing, requests), the knowledge, intelligence ×3, output-templates and asset-detail pages, the branding and index-status providers, the connection-skills panel, the setup checklist, the review-gate draft-viewer role grant, the library register's asset and viewer affordances (`FullScreenViewer` / `CollectionsStrip` only accepted a scalar `userRole` prop — both now take `userRoles`), and the ticket page's admin / requester affordances — and **23 server-side singular-role reads**: eleven routes that never selected `roles` (tickets/comment, links/propose, equipment-bridge, data-export/structured, companies/quality-manual, flows/read, graph/shape, graph/mentions, projects/cost-docs, codebook/import, intake/upload), the create-user route's Admin-grant escalation guard (it read the caller's headline while the general gate read the collection), the controller notify pools in cron/maintenance, data-export/run and intake/upload (`.in("role", …)`), and the four pool resolvers in `lib/` (`reviewControl` role-designated reviewers, `acknowledgments` role assignees, `ticketRouting` status pools, `dataRestore` placeholder seeding — which discarded the backup's `roles` array). All are converted: client gates through `hasAnyRole` / `roles`; server routes and pools through the new `lib/roleHeld.ts` (`heldRoles`, `memberHoldsAny`, and `roleFilter` — the `role.in.(…),roles.ov.{…}` PostgREST idiom `lib/reviewCycles.ts` already proved, with quoted values so `Engineer-1` survives); restore keeps every non-privileged role the backup held (`restoredMemberRoles`) and mirrors the headline (`restoredMemberHeadline`). A census test (`lib/__tests__/sweepRoundC1b.test.ts`) pins every converted file against the headline-authority patterns so they cannot return. Deliberately left on the headline: display uses (`{role} Console`, audit payload `userRole`, the requests console's card lens and its `assignedTo` default filter) and the resolved-membership sentinel (`activeRole &&`) that only says the membership has loaded.
...
- Residual → done the same day in Round C1b (see the `ADD-1` amendment): the full census found 42 headline-keyed client gates across 27 files and 23 server-side singular-role reads, all converted and pinned by `lib/__tests__/sweepRoundC1b.test.ts`.
...
**Resolution (2026-09-02, Phase 6 severity sweep, Round B).** Every latest definition that still read `org_members.role` alone now reads the collection: `is_org_admin` (feeds the Admin-conferral clause of `org_members_update`/`org_members_write` and the branding policies) → `caller_holds_any_role(p_org, ARRAY['Admin'])`; `teams_admin_write` / `team_members_admin_write` (`SURF-16`), `checkout_messages_own_update`, `orgs_admin_write` (only ever defined in `schema.sql` — dropped and recreated) and the eight side-table write policies (`org_ai_instructions`, `document_related_resources`, `library_numbering`, `proposed_links`, `asset_aliases`, `codebook_entries`, `codebook_config`, `entity_mentions`) → `caller_holds_any_role(org_id, …)` with their original role sets, existing tables only; `can_manage_node` evaluates every held role (`DOCACL-1`); `prevent_last_admin_removal` counts collection-held Admins. `caller_holds_any_role` (20261045) is the funnel.
- Done-when: ✓ no latest policy or function reads the headline without the collection (the census is in the recon record); ✓ one helper carries the rule.
...
**Resolution (2026-09-02, Phase 6 severity sweep, Round B).** `org_members_sync_role_collection` — a `BEFORE INSERT OR UPDATE OF role, roles` trigger — keeps the mirror and the collection consistent at the database: an empty `roles[]` is seeded from the headline, the headline is always a member of the collection, and the headline is re-derived as the highest-ranked held role via `role_rank(text)`, which mirrors `ROLE_RANK` exactly (test-pinned against `lib/roleCapabilities.ts`, never reordered per `DEC-2`). Idempotent for correct rows; a one-shot repair inside the migration handles rows written before it (the two non-lossy branches of `20261024`). The trigger sorts before the last-admin guard, so that guard sees the synced row. A writer that wants to demote must remove the role from the collection — the headline is derived, never authoritative (the app already writes both).
- Done-when: ✓ no row can be born with `roles = '{}'`; ✓ the invariant lives where the row is written.
- Migration: `20261046` — **applied & verified live 2026-09-02** (12-point probe all true; inventory recorded BEFORE apply: 0 additive holders under a lower headline, 0 empty / 0 headline-missing / 0 mis-ranked collections, 0 terminal-status documents).. Tests: `lib/__tests__/rpSweepMigrations.test.ts` (shape + line-diffs against the live predecessors), `lib/__tests__/sweepRoundB.test.ts`.
