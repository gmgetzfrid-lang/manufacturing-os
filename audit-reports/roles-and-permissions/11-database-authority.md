...
**Resolution (2026-09-17, Round E).** The census is now a TEST — `lib/__tests__/authorityCensus.test.ts` — so the headline-only idioms the audit found cannot reappear (a novel mixed body still needs a reviewer; see residual). It replays `supabase/schema.sql` + every numbered migration in order (statements in textual order, `DROP FUNCTION` / `DROP POLICY` honoured, and the `DO`-loop policy rewrites of `20261045` / `20261046` parsed as creates keyed by their `FOREACH … ARRAY[…]` / `VALUES (tbl, pol, roles)` lists) and classifies every live function and policy that touches `org_members` into one of three families: **additive** (reads `roles`, or calls a collection funnel — `is_org_controller`, `caller_holds_any_role`, `is_org_admin`, `is_org_admin_or_manager`, `org_capability_allows` / `_for`, `acl_index_denies`), **membership-only** (`uid` / `status` only), or **headline-only** (the bare `role` column and nothing collection-shaped; `v_role`, `author_role` and the JSON key `'role'` are not reads of the column). The test **fails if any live definition is headline-only**, and **fails if any live body — additive or not — carries one of the retired headline idioms** (`SELECT role INTO v_role`, `v_role IN (…)`, `v_role = 'Admin' | 'DocCtrl'`; the pin is negative-tested against the pre-DEC-2 `node_visible` body in `20261037`, which carries two of them); pins the seven funnels to the additive family (the five that read `roles` directly do, `is_org_admin` and the 3-arg `org_capability_allows` delegate to a funnel) and `is_org_controller`'s check byte-shape; pins the six DEC-2 sites additive with final definitions at or after `20261040`, and pins the conversion itself on the migrations that performed it — `20261040` defines the four first-wave sites with additive bodies and does not touch `node_visible`; `20261041` defines `node_visible` (additive, via `is_org_controller`) and nothing else; `20261040` replays before `20261041` — so `node_visible`'s final definition may later move to a newer file without breaking the pin, and none of the six carries `SELECT role INTO v_role` any more; pins `20261024`'s two-branch backfill ahead of every conversion; and pins `ROLE_RANK` byte-for-byte. Negative-tested by deleting `20261046`'s DO loop from a scratch copy: the test named all eight side-table `_write` policies as headline-only. `PRINT_AUTHORITY_CENSUS=<path>` writes the table below.
...
**Done-when.** (1) ✓ with one caveat: the six pinned sites evaluate the controller tier through the collection — four via `is_org_controller`, and two inline-additive over the collection (`publish_revision` on its named actor, as `OWN-3` recorded, and `user_can_publish_on_library`, whose funnel the test pins as `roles`), pinned by the test. The `99-fix-sequencing.md` "one at a time" order was **not** followed literally: four sites landed together in `20261040` (Phase 5 / `OWN-3`, before this package), not one per migration. The two load-bearing halves are met and both pinned — `DB-3`'s backfill first (item 2), and `node_visible` last and separately (`20261040` converts the four and does not touch `node_visible`; `20261041` converts `node_visible` alone, and replays after it). (2) ✓ `DB-3`'s backfill landed first (`20261024`, applied live 2026-08-24) — the test pins its two `UPDATE` branches and its position ahead of every additive conversion. (3) ✓ the widening inventory was run and recorded (`OWN-3`, operator-run 2026-09-01, aggregate: **0** members holding Admin/DocCtrl additively under a higher headline). (4) ✓ `ROLE_RANK` byte-identical — pinned by the test.

**Scope / residual.** No migration; nothing was converted here (`DEC-31` — the census is the deliverable, the conversions landed in Phases 5–6 and are what it certifies). The classifier is textual: a body that reads `roles` for one decision and `role` alone for another classifies additive. The global idiom pin catches the shapes the audit actually found (`SELECT role INTO v_role`, `v_role IN (…)`, `v_role = 'Admin'`) in every live body regardless of family; a NOVEL mixed idiom would pass the classifier and still needs a reviewer. The known mixed case (`node_visible` pairs its single-role `acl_subject_in_bucket` call with the `unnest(v_roles)` pass) is covered by the DEC-2 site pins. A policy created dynamically is classified by its whole DO block. Any package that adds a headline-only read to a migration will fail this test; that is the point.
...
**Resolution (2026-09-17, Round E).** The prospective hazard had become real: on the day of retirement `supabase/REMEDIATION_APPLY_ALL.sql` forked from the live sequence at **7 of its 23 definitions** (comparison basis: each definition's text with comments stripped, whitespace collapsed and case folded, against the final definition in the numbered sequence) — functions `doc_is_visible` (live `20261037`), `can_manage_node` (`20261046`), `documents_guard_access_change` (`20261044`), `is_org_admin` (`20261046`) and policies `projects_visibility_select` (`20260913`), `org_members_update` / `org_members_write` (`20260817`) — so a re-run would have reverted `can_manage_node` to the headline-only body and `is_org_admin` to `role = 'Admin'`. The other 16 — `is_org_controller`, `my_project_ids`, `acl_subject_has_action`, `is_org_admin_or_manager`, the `documents_guard_access` trigger and eleven policies — still matched their live definitions modulo case and whitespace: forks-in-waiting, not forks. All three out-of-sequence scripts are retired as **guarded no-ops**: the named file; `supabase/APPLY_roles-and-permissions_2026-08-24.sql` (same mechanism, surfaced by the new test — `publish_revision` frozen at `20261019` vs live `20261049`, `org_capability_allows` frozen at `20261025` vs the `20261052` resource-dimension wrapper, so a re-run would have silently switched request-type-scoped rules off, and the `document_shares` INSERT / UPDATE policies frozen before `20261026` / `20261037` — 4 of its 8 definitions at retirement, 5 once `20261066` lands because it redefines `document_shares_org_select`); and `supabase/migrations/CATCHUP_2026-05-28.sql` (3 of its 31 distinct definitions — the `checkout_messages` policies; `checkout_messages_own_update` reads the collection since `20261046`). Each stub is a header comment naming what it was, where it forked, and where the truth lives, plus a single `DO $$ … RAISE EXCEPTION 'RETIRED (DB-8) …' $$` so a paste refuses loudly instead of reverting silently; the stubs are kept (rather than the files deleted) so a bookmark or runbook that still points at one meets the refusal. The original texts remain in git history. `lib/__tests__/migrationSourceOfTruth.test.ts` walks every `.sql` under `supabase/` and fails if any file outside `supabase/migrations/NNNNNNNN_*.sql` (`schema.sql`, the baseline, excepted) defines a function, policy or trigger that the numbered sequence defines — "byte-identical today" is not an exemption, that is exactly the fork-in-waiting this finding describes — and asserts the three stubs carry the guard and no DDL. Run against the pre-change tree it named all **62** overlapping definitions (23 + 8 + 31 — the test flags any overlap with the sequence, forked or not); it passes after.
