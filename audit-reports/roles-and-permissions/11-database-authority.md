...
**Done-when.** (1) ✓ with one caveat: the five sites evaluate the controller tier through the collection — four via `is_org_controller`, `publish_revision` inline-additive on its named actor (as `OWN-3` recorded), pinned by the test. The `99-fix-sequencing.md` "one at a time" order was **not** followed literally: four sites landed together in `20261040` (Phase 5 / `OWN-3`, before this package), not one per migration. The two load-bearing halves are met and both pinned — `DB-3`'s backfill first (item 2), and `node_visible` last and separately (`20261040` converts the four and does not touch `node_visible`; `20261041` converts `node_visible` alone, and replays after it). (2) ✓ `DB-3`'s backfill landed first (`20261024`, applied live 2026-08-24) — the test pins its two `UPDATE` branches and its position ahead of every additive conversion. (3) ✓ the widening inventory was run and recorded (`OWN-3`, operator-run 2026-09-01, aggregate: **0** members holding Admin/DocCtrl additively under a higher headline). (4) ✓ `ROLE_RANK` byte-identical — pinned by the test.
